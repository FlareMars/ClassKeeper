 ...
 * 处理最新的通知反馈
 */
public class ProcessNoticeFeedback implements IProcessMsg {
    @Override
    public void process(JSONObject content, Context context, UserObject thisUser) {
        try {
            List<NoticeTaskObject> taskObjects = DataSupport.where("noticeId=?", content.getString(MessageConst.CONTENT_TARGET_ID))
                    .find(NoticeTaskObject.class);
            NoticeTaskObject taskObject;
            if (taskObjects.size() > 0) {
                taskObject = taskObjects.get(0);
            } else {
                return;
            }
            NoticeFeedbackObject tempFeedback = DataSupport.where("noticetaskobject_id=? and whose=?",
                    String.valueOf(taskObject.getId()),content.getString(MessageConst.CONTENT_FROM_ID)).find(NoticeFeedbackObject.class).get(0);
            tempFeedback.setDone(true);
            tempFeedback.update(tempFeedback.getId());
            taskObject.setNewFeedback(true);
            taskObject.update(taskObject.getId());

            //添加通知组反馈
            Cursor cursor = DataSupport.findBySQL("select noticegroupobject_id from noticetaskobject where id=?",String.valueOf(taskObject.getId()));
            long targetId = 0;
            ...
            if (recentNoticeGroupMsgs.size() == 0) {
                ...
                recentNoticeGroupMsg.setTime(new Date(content.getLong(MessageConst.CONTENT_TIME)));
            ...
            List<RecentMessageObject> recentMessageObjects = DataSupport.where("type=0 and targetId=? and classobject_id=? and userobject_id=?",
                    String.valueOf(noticeGroupObject.getId()),String.valueOf(classObject.getId()),String.valueOf(mineObject.getId())).
...
//            NotificationUtils.INSTANCE.toNotify("有新的通知阅读反馈");
